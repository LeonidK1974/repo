...
vector<Spending> LoadFromXml(istream& input) {
  // Реализуйте эту функцию с помощью библиотеки xml.h
	vector<Spending>  vector_of_spendings;
	Document doc = Load(input);
	Node root = doc.GetRoot();
	for (const Node& node : root.Children())
	{
		vector_of_spendings.push_back({ node.AttributeValue<string>("category"),  node.AttributeValue<int>("amount") });
	}
	return vector_of_spendings;
}
