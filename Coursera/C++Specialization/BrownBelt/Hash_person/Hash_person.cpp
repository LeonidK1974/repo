...
struct AddressHasher {
  // реализуйте структуру
	...
	size_t operator()(const Address& address) const {
		...
		const size_t coef = 2053;

		return (
			coef * coef * shash(address.city) +
			coef * shash(address.street) +
...
struct PersonHasher {
  // реализуйте структуру
	...
	size_t operator()(const Person& person) const {
		...
		return (
			coef * coef * coef * shash(person.name) +
			coef * coef * ihash(person.height) +
			coef * dhash(person.weight) +
