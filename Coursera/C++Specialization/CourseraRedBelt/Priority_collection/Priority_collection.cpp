...
public:
  ...
  Id Add(T object) {
	  collection_elems.push_back(move(object));
	  object_priorities.push_back(0);
	  priorities[{0, object_priorities.size() - 1}] = prev(collection_elems.end());
  ...
  pair<T, int> PopMax() {
	  ...
	  int priority = it_pr_last->first.first;
...
private:
  // Приватные поля и методы
	list<T>	collection_elems;
	vector<int> object_priorities;
	map<pair<int, Id>, typename list<T>::iterator> priorities;
};
